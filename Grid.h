...
#include "utils/Globals.h"   // for SharedAlias, SharedDipole, Palette, etc.

#include <cmath>             // for std::atan2, std::cos, std::sin
#include <QRectF>            // base class
#include <QPainter>          // QPainter
#include <QPixmap>           // QPixmap
#include <QPointF>           // QPointF
#include <QLine>             // QLine
#include <QLineF>            // QLineF
#include <QBrush>            // QBrush
#include <QPen>              // QPen
#include <QSet>              // QSet<>
#include <QString>           // QString
#include <QColor>            // QColor
#include <QPaintEvent>       // QPaintEvent (for the render() signature)
...
private:
    static constexpr int    DEFAULT_ZOOM_INDEX = 5;
    static constexpr int    NUMBER_OF_ZOOM_STATES = 14;
    static constexpr int    MAJ_ZOOM_INDEX = NUMBER_OF_ZOOM_STATES-1;
    static constexpr float  ZOOM_STATES[NUMBER_OF_ZOOM_STATES] = {
...
public:
    ...
    inline float toScreenX(const float x) const {
        return (x - left())*zoom();
    }
    inline float toScreenY(const float y) const {
        return (y - top())*zoom();
    }
    inline QPointF toScreen(const QPointF &pos) const {
        return QPointF(toScreenX(pos.x()), toScreenY(pos.y()));
    }
    inline QLine toScreen(const QLineF &line) const {
        return QLine(toScreenX(line.x1()), toScreenY(line.y1()), toScreenX(line.x2()), toScreenY(line.y2()));
    }
    inline QRectF toScreen(const QRectF &rect) const {
        return QRectF(toScreenX(rect.x()), toScreenY(rect.y()), rect.width()*zoom(), rect.height()*zoom());
    }
    ...
    void drawAliasLabel(const QPointF &center, const QString &label, const QColor &color);
